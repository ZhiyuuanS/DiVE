...
class VideoRecorder:
  ...
  def _save(self):
    ...
    imageio.mimsave(filename, self._frames, codec='libx264', fps=10)
...
class EpisodeRecorder:
  ...
  def step(self, action):
    ...
    description_info = {'text_description': text_description, 
                        'inventory': info['inventory'], 
                        'time_step': self._env._step}
    from LLM.descriptor import SimplifiedStateDescriptor
    descriptor = SimplifiedStateDescriptor()
    description = descriptor.describe(description_info)
    transition['description'] = description

    daylights = self._env._world.daylight
    transition['daylights'] = daylights
    transition['text_description'] = text_description
